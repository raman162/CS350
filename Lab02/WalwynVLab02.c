...
int main() {
	printf("CS 350 Lab 2 for %s\n\n", "Raman Walwyn-Venugopal Section 01");  // <-- Fill in
	int value, base;
	int digits[ARRAYLEN];
	...
	while (value >= 1 && base >= 2) {
	  ...
	  convert_to_base(value, base,digits, ARRAYLEN );
...
void convert_to_base(int value, int base, int digits[], int len) {
	...
	while(remain > 0 && position >=0){
	  ...
	  represented = represented + digits[position] * pow(base, i++);
	  remain = remain/base;
	  position--;
	}

	if (remain>0){
	  printf("%d is too large for %d digits in base %d; %d is leftover ",value, len, base, value-represented);
	}
